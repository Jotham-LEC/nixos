...
;; Must be set before evil loads, so it can't live in config.el. Makes j/k and
;; friends move by screen line under `visual-line-mode', as C-n/C-p already do
;; without evil. Inert when `:editor evil' is off.
...
(doom! :input
       ...
       direnv            ; load .envrc environments into Emacs
       ;;docker
       ...
       (org +crypt +pretty +present +dragndrop +pandoc) ; organize your plain life in plain text
       ...
       (web +css +html +lsp +tree-sitter) ; the tubes
       yaml              ; JSON, but readable
       ;;zig               ; C, but simpler

       :email
       (mu4e +org +mbsync) ; UI in lisp/my-mail.el; backend in modules/services/mail/
       ;;notmuch
       ;;(wanderlust +gmail)
       ...
       :config
       ;; literate
       (default +bindings +smartparens)) ; sane defaults and keybinds
